			...
			<div id="layout" class="{{$txt['107419::70925_layout_class']}}">
				...
				{{if !( $xml->getItem("action/set_render/item")) or ( $xml->getItem("action/set_render/item/@name")  == (string) 'content')}}
					<div {{assign "classAttributeTmp2" ""}}  id="content"{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}span9{{/capture}}{{if $xml->getItem("page/@type") == (string) 'projector'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_projector_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'search'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_search_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'main'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_main_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'navigation'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_navigation_page_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-list'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_list_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-item'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_item_c_class']}}{{/capture}}{{else}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_otherwise_c_class']}}{{/capture}}{{/if}}  class="{{$classAttributeTmp2}}">
						...
						<h1 class="categories-list_label big_label">{{$txt['107766::30003_01']}}
						</h1>
						<div class="categories-list_wrapper">
							{{if  $xml->getItem("page/categories-list/@mode") == (string)'2'}}
								<ul class="categories-list_level1">
									{{foreach from=$xml->getList("/shop/navigation2/item") item=loop246}}
										<li class="categories-list_level1">
											{{if  $loop246->getItem("@link")}}
												<a class="categories-list_level1" href="{{$loop246->getItemEscape("@link")}}" title="{{$loop246->getItemEscape("@name")}}">{{$loop246->getItem("@name")}}</a>
											{{else}}
												<span class="categories-list_level1">{{$loop246->getItem("@name")}}</span>
											{{/if}}
											{{if  $loop246->getItem("item")}}
												<span {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories d-sm-none{{/capture}}  class="{{$classAttributeTmp3}}">{{$txt['107766::40002_000']}}</span>
												<ul class="categories-list_level2">
													{{foreach from=$loop246->getList("item") item=loop266}}
														<li class="categories-list_level2">
															{{if  $loop266->getItem("@link")}}
																<a class="categories-list_level2" href="{{$loop266->getItemEscape("@link")}}" title="{{$loop266->getItemEscape("@name")}}">{{$loop266->getItem("@name")}}</a>
															{{else}}
																<span class="categories-list_level2">{{$loop266->getItem("@name")}}</span>
															{{/if}}
															{{if  $loop266->getItem("item")}}
																<span {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories d-sm-none{{/capture}}  class="{{$classAttributeTmp3}}">{{$txt['107766::40002_000']}}</span>
																<ul class="categories-list_level3">
																	{{foreach from=$loop266->getList("item") item=loop286}}
																		<li class="categories-list_level3">
																			{{if  $loop286->getItem("@link")}}
																				<a class="categories-list_level3" href="{{$loop286->getItemEscape("@link")}}" title="{{$loop286->getItemEscape("@name")}}">{{$loop286->getItem("@name")}}</a>
																			{{else}}
																				<span class="categories-list_level3">{{$loop286->getItem("@name")}}</span>
																			{{/if}}
																		</li>
																	{{/foreach}}
																</ul>
							...
							{{else}}
								<ul class="categories-list_level1">
									{{foreach from=$xml->getList("/shop/navigation/item") item=loop316}}
										<li class="categories-list_level1">
											{{if  $loop316->getItem("@link")}}
												<a class="categories-list_level1" href="{{$loop316->getItemEscape("@link")}}" title="{{$loop316->getItemEscape("@name")}}">{{$loop316->getItem("@name")}}</a>
											{{else}}
												<span class="categories-list_level1">{{$loop316->getItem("@name")}}</span>
											{{/if}}
											{{if  $loop316->getItem("item")}}
												<span {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories d-sm-none{{/capture}}  class="{{$classAttributeTmp3}}">{{$txt['107766::40002_000']}}</span>
												<ul class="categories-list_level2">
													{{foreach from=$loop316->getList("item") item=loop336}}
														<li class="categories-list_level2">
															{{if  $loop336->getItem("@link")}}
																<a class="categories-list_level2" href="{{$loop336->getItemEscape("@link")}}" title="{{$loop336->getItemEscape("@name")}}">{{$loop336->getItem("@name")}}</a>
															{{else}}
																<span class="categories-list_level2">{{$loop336->getItem("@name")}}</span>
															{{/if}}
															{{if  $loop336->getItem("item")}}
																<span {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}more_categories d-sm-none{{/capture}}  class="{{$classAttributeTmp3}}">{{$txt['107766::40002_000']}}</span>
																<ul class="categories-list_level3">
																	{{foreach from=$loop336->getList("item") item=loop356}}
																		<li class="categories-list_level3">
																			{{if  $loop356->getItem("@link")}}
																				<a class="categories-list_level3" href="{{$loop356->getItemEscape("@link")}}" title="{{$loop356->getItemEscape("@name")}}">{{$loop356->getItem("@name")}}</a>
																			{{else}}
																				<span class="categories-list_level3">{{$loop356->getItem("@name")}}</span>
																			{{/if}}
																		</li>
																	{{/foreach}}
																</ul>
