				...
				{{if !( $xml->getItem("action/set_render/item")) or ( $xml->getItem("action/set_render/item/@name")  == (string) 'content')}}
					<div {{assign "classAttributeTmp2" ""}}  id="content"{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}span9{{/capture}}{{if $xml->getItem("page/@type") == (string) 'projector'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_projector_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'search'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_search_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'main'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_main_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'navigation'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_navigation_page_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-list'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_list_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-item'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_item_c_class']}}{{/capture}}{{else}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_otherwise_c_class']}}{{/capture}}{{/if}}  class="{{$classAttributeTmp2}}">
						...
						{{capture name="setSerialNumber" assign="setSerialNumber"}}
							{{$txt['69371::57649_setSerialNumber']}}
						{{/capture}}
						<div class="client_order">
							<h1 class="big_label">
								<span>{{$txt['69371::54068_linksclosed']}}</span>
							</h1>
							{{if  $xml->getItem("/shop/page/returns/item")}}
								<table {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}orders{{/capture}} cellspacing="0" cellpadding="0"{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}ui-responsive table-stroke ui-table ui-table-reflow orders{{/capture}}  class="{{$classAttributeTmp3}}">
									<thead>
										<tr id="orders_header">
											<td class="ord3" id="orders_head_1">{{$txt['69371::51900_001']}}
											</td>
											<td class="ord3" id="orders_head_2">{{$txt['69371::51900_002']}}
											</td>
											<td class="ord3" id="orders_head_3">{{$txt['69371::51900_003']}}
											</td>
											<td class="ord3" id="orders_head_5">{{$txt['69371::51900_005']}}
											</td>
											<td class="ord3" id="orders_head_6">{{$txt['69371::51900_006']}}
											</td>
										</tr>
									</thead>
									<tbody>
										{{foreach from=$xml->getList("/shop/page/returns/item") item=loop316}}
											<tr>
												<td class="ord">
													{{if $setSerialNumber}}
														{{$loop316->getItemEscape("@number")}}
													{{else}}
														{{$loop316->getItemEscape("@id")}}
													{{/if}}
												</td>
												<td>{{$loop316->getItemEscape("@date")}}
												</td>
												<td class="ord">{{$loop316->getItemEscape("@cost_formatted")}}
												</td>
												<td>
													{{if  $loop316->getItem("@status_name")  neq (string) ''}}
														{{$loop316->getItem("@status_name")}}
													{{elseif  $loop316->getItem("@status") == (string)1}}
														{{$txt['69371::status_1']}}
													{{elseif  $loop316->getItem("@status") == (string)2}}
														{{$txt['69371::status_2']}}
													{{elseif  $loop316->getItem("@status") == (string)3}}
														{{$txt['69371::status_3']}}
													{{elseif  $loop316->getItem("@status") == (string)13}}
														{{$txt['69371::status_13']}}
													{{elseif  $loop316->getItem("@status") == (string)14}}
														{{$txt['69371::status_14']}}
													{{elseif  $loop316->getItem("@status") == (string)15}}
														{{$txt['69371::status_15']}}
													{{elseif  $loop316->getItem("@status") == (string)16}}
														{{$txt['69371::status_16']}}
													{{elseif  $loop316->getItem("@status") == (string)17}}
														{{$txt['69371::status_17']}}
													{{elseif  $loop316->getItem("@status") == (string)18}}
														{{$txt['69371::status_18']}}
													{{/if}}
												</td>
												<td class="ord">
													<a class="orders_more" data-tablet-class="btn-small" data-mobile-class="btn-small" href="{{$loop316->getItemEscape("@url")}}">{{$txt['69371::51900_018']}}</a>
												</td>
											</tr>
										{{/foreach}}
									</tbody>
							...
							{{else}}
								<div class="return_list_noitem menu_messages_message" id="return_list_noitem">{{$txt['69371::return_list_norma']}}
								</div>
							{{/if}}
						</div>
					</div>
