					...
					<div {{assign "classAttributeTmp2" ""}}  id="content"{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}span9{{/capture}}{{if $xml->getItem("page/@type") == (string) 'projector'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_projector_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'search'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_search_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'main'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_main_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'navigation'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_navigation_page_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-list'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_list_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-item'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_item_c_class']}}{{/capture}}{{else}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_otherwise_c_class']}}{{/capture}}{{/if}}  class="{{$classAttributeTmp2}}">
						...
						{{if !( $xml->getItem("page/@type") == (string)'affiliate-not-active')}}
							<div {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}affiliate_clients_label{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}big_label affiliate_clients_label{{/capture}}  class="{{$classAttributeTmp3}}">
								<span class="affiliate_clients_label">{{$txt['59424::affiliate_clients_label']}}</span>
							</div>
							{{if count( $xml->getList("page/clients_list/login"))}}
								<div class="affiliate_clients_desc">
									<p class="affiliate_clients_desc">{{$txt['59424::affiliate_clients_desc']}}
									</p>
								</div>
								<div class="affiliate_clients">
									<div class="affiliate_clients_sub">
										<table {{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}affiliate_clients{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}ui-responsive table-stroke ui-table ui-table-reflow affiliate_clients{{/capture}}  class="{{$classAttributeTmp4}}">
											<thead>
												<tr>
													<th id="affiliate_clients_th1">{{$txt['59424::affiliate_clients_th1']}}
													</th>
													<th id="affiliate_clients_th3">{{$txt['59424::affiliate_clients_th3']}}
													</th>
												</tr>
											</thead>
											<tbody>
												{{foreach from=$xml->getList("page/clients_list/login") item=loop300}}
													<tr {{assign "classAttributeTmp5" ""}}{{if $loop300@iteration % 2}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}affiliate_clients_row1{{/capture}}{{else}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}affiliate_clients_row2{{/capture}}{{/if}}  class="{{$classAttributeTmp5}}">
														<td class="affiliate_clients_first">
															<span class="affiliate_clients_name">
																<a data-mobile-class="btn-small" href="{{$loop300->getItemEscape("@link")}}">{{$loop300->getItemEscape("@nr")}}</a></span>
														</td>
														<td>{{$loop300->getItemEscape("@date")}}
														</td>
													</tr>
												{{/foreach}}
											</tbody>
							...
							{{else}}
								<div class="affiliate_clients_desc_null">{{$txt['59424::affiliate_clients_desc_null']}}
								</div>
							{{/if}}
						{{/if}}
						<!--Klienci zarejestrowani z PP - Stronicowanie (affiliate_clients_paging, 91987.1)-->
						{{if count( $xml->getList("page/clients_list/navigation/item"))}}
							<ul class="pagination pull-right">
								{{if count( $xml->getList("page/clients_list/navigation/item"))  gt (string) 1}}
									{{if  $xml->getItem("page/clients_list/navigation/prev")}}
										<li>
											<a href=" {{$xml->getItem("page/clients_list/navigation/path/@value")}}{{$xml->getItemEscape("page/clients_list/navigation/prev/@counter")}}">{{$txt['91987::affiliate_clients_list_002']}}</a>
										</li>
									{{/if}}
									{{if !( $xml->getItem("page/clients_list/navigation/prev"))}}
										<li class="disabled">
											<span>{{$txt['91987::affiliate_clients_list_002a']}}</span>
										</li>
									{{/if}}
									{{foreach from=$xml->getList("page/clients_list/navigation/item") item=loop354}}
										{{if  $loop354->getItem("@gap")}}
											<li class="disabled">
												<span>{{$txt['91987::affiliate_clients_list_007']}}</span>
											</li>
										{{/if}}
										{{if !( $loop354->getItem("@type")  == (string) 'current') and !( $loop354->getItem("@gap"))}}
											<li>
												<a href="{{$loop354->getItem("../path/@value")}}{{$loop354->getItemEscape("@counter") - 1}}">{{$loop354->getItemEscape("@counter")}}</a>
											</li>
										{{/if}}
										{{if  $loop354->getItem("@type")  == (string) 'current'}}
											<li class="active">
												<span>{{$loop354->getItemEscape("@counter")}}</span>
											</li>
										{{/if}}
									{{/foreach}}
									{{if  $xml->getItem("page/clients_list/navigation/next")}}
										<li>
											<a href="{{$xml->getItem("page/clients_list/navigation/path/@value")}}{{$xml->getItemEscape("page/clients_list/navigation/next/@counter")}}">{{$txt['91987::affiliate_clients_list_003']}}</a>
										</li>
									{{/if}}
									{{if !( $xml->getItem("page/clients_list/navigation/next"))}}
										<li class="disabled">
											<span>{{$txt['91987::affiliate_clients_list_003a']}}</span>
										</li>
									{{/if}}
								{{/if}}
							</ul>
