				...
				{{if !( $xml->getItem("action/set_render/item")) or ( $xml->getItem("action/set_render/item/@name")  == (string) 'content')}}
					<div {{assign "classAttributeTmp2" ""}}  id="content"{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}span9{{/capture}}{{if $xml->getItem("page/@type") == (string) 'projector'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_projector_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'search'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_search_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'main'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_main_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'navigation'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_navigation_page_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-list'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_list_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-item'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_item_c_class']}}{{/capture}}{{else}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_otherwise_c_class']}}{{/capture}}{{/if}}  class="{{$classAttributeTmp2}}">
						...
						{{capture name="login" assign="login"}}
							{{$txt['108106::client_zone_login']}}
						{{/capture}}
						{{capture name="loginedit" assign="loginedit"}}
							{{$txt['108106::client_zone_loginedit']}}
						{{/capture}}
						{{capture name="address" assign="address"}}
							{{$txt['108106::client_zone_address']}}
						{{/capture}}
						{{capture name="orders_opened" assign="orders_opened"}}
							{{$txt['108106::client_zone_orders_opened']}}
						{{/capture}}
						{{capture name="orders_closed" assign="orders_closed"}}
							{{$txt['108106::client_zone_orders_closed']}}
						{{/capture}}
						{{capture name="affiliate" assign="affiliate"}}
							{{$txt['108106::client_zone_affiliate']}}
						{{/capture}}
						{{capture name="rma" assign="rma"}}
							{{$txt['108106::client_zone_rma']}}
						{{/capture}}
						<div class="sitemap_page_sub">
							<h1 class="big_label">{{$txt['108106::sitemap_header']}}
							</h1>
							<div class="sitemap_mainpage_wrapper">
								<label>{{$txt['108106::zone_tit_1']}}
								</label>
								<ul>
									{{foreach from=$xml->getList("page/sitemap/item[@id='mainpage']/item") item=loop276}}
										<li>
											<a href="{{$loop276->getItemEscape("@link")}}" title="{{$loop276->getItemEscape("@name")}}">{{$loop276->getItemEscape("@name")}}</a>
										</li>
									{{/foreach}}
								</ul>
							</div>
							<div class="sitemap_searchpage_wrapper">
								<label>{{$txt['108106::zone_tit_2']}}
								</label>
								<ul>
									{{foreach from=$xml->getList("page/sitemap/item[@id='searchpage']/item") item=loop292}}
										<li>
											<a href="{{$loop292->getItemEscape("@link")}}" title="{{$loop292->getItemEscape("@name")}}">{{$loop292->getItemEscape("@name")}}</a>
										</li>
									{{/foreach}}
								</ul>
							</div>
							<div class="sitemap_clientpage_wrapper">
								<label>{{$txt['108106::zone_tit_3']}}
								</label>
								<ul>
									{{foreach from=$xml->getList("page/sitemap/item[@id='clientpage']/item") item=loop309}}
										<li>
											<a href="{{$loop309->getItemEscape("@link")}}" {{if $loop309->getItem("@id") == (string)'login'}} title="{{$login}}"{{$login}}{{elseif $loop309->getItem("@id") == (string)'loginedit'}} title="{{$loginedit}}"{{$loginedit}}{{elseif $loop309->getItem("@id") == (string)'address'}} title="{{$address}}"{{$address}}{{elseif $loop309->getItem("@id") == (string)'orders_opened'}} title="{{$orders_opened}}"{{$orders_opened}}{{elseif $loop309->getItem("@id") == (string)'orders_closed'}} title="{{$orders_closed}}"{{$orders_closed}}{{elseif $loop309->getItem("@id") == (string)'affiliate'}} title="{{$affiliate}}"{{$affiliate}}{{elseif $loop309->getItem("@id") == (string)'rma'}} title="{{$rma}}"{{$rma}}{{/if}}></a>
										</li>
									{{/foreach}}
								</ul>
							</div>
							{{if  $xml->getItem("page/sitemap/item/item[@id='categories']")}}
								<div class="sitemap_categories_wrapper">
									...
									<ul>
										{{foreach from=$xml->getList("page/sitemap/item/item[@id='categories']/item") item=loop327}}
											<li>
												<a href="{{$loop327->getItemEscape("@link")}}" title="{{$loop327->getItemEscape("@name")}}">{{$loop327->getItemEscape("@name")}}</a>
												{{if  $loop327->getItem("item")}}
													<ul>
														{{foreach from=$loop327->getList("item") item=loop333}}
															<li>
																<a href="{{$loop333->getItemEscape("@link")}}" title="{{$loop333->getItemEscape("@name")}}">{{$loop333->getItemEscape("@name")}}</a>
																{{if  $loop333->getItem("item")}}
																	<ul>
																		{{foreach from=$loop333->getList("item") item=loop339}}
																			<li>
																				<a href="{{$loop339->getItemEscape("@link")}}" title="{{$loop339->getItemEscape("@name")}}">{{$loop339->getItemEscape("@name")}}</a>
																				{{if  $loop339->getItem("item")}}
																					<ul>
																						{{foreach from=$loop339->getList("item") item=loop345}}
																							<li>
																								<a href="{{$loop345->getItemEscape("@link")}}" title="{{$loop345->getItemEscape("@name")}}">{{$loop345->getItemEscape("@name")}}</a>
																							</li>
																						{{/foreach}}
																					</ul>
																				{{/if}}
							...
							{{if  $xml->getItem("page/sitemap/item/item[@id='producers']")}}
								<div class="sitemap_categories_wrapper">
									...
									<ul>
										{{foreach from=$xml->getList("page/sitemap/item/item[@id='producers']/item") item=loop374}}
											<li>
												<a href="{{$loop374->getItemEscape("@link")}}" title="{{$loop374->getItemEscape("@name")}}">{{$loop374->getItemEscape("@name")}}</a>
											</li>
										{{/foreach}}
									</ul>
								</div>
							...
							{{if  $xml->getItem("page/sitemap/item/item[@id='traits']")}}
								<div class="sitemap_traits_wrapper">
									...
									<ul>
										{{foreach from=$xml->getList("page/sitemap/item/item[@id='traits']/item") item=loop390}}
											<li>
												<a href="{{$loop390->getItemEscape("@link")}}" title="{{$loop390->getItemEscape("@name")}}">{{$loop390->getItemEscape("@name")}}</a>
												{{if  $loop390->getItem("item")}}
													<ul>
														{{foreach from=$loop390->getList("item") item=loop396}}
															<li>
																<a href="{{$loop396->getItemEscape("@link")}}" title="{{$loop396->getItemEscape("@name")}}">{{$loop396->getItemEscape("@name")}}</a>
															</li>
														{{/foreach}}
													</ul>
												{{/if}}
							...
							{{if  $xml->getItem("page/sitemap/item/item[@id='series']")}}
								<div class="sitemap_series_wrapper">
									...
									<ul>
										{{foreach from=$xml->getList("page/sitemap/item/item[@id='series']/item") item=loop415}}
											<li>
												<a href="{{$loop415->getItemEscape("@link")}}" title="{{$loop415->getItemEscape("@name")}}">{{$loop415->getItemEscape("@name")}}</a>
											</li>
										{{/foreach}}
									</ul>
								</div>
							...
							{{if  $xml->getItem("page/sitemap/item/item[@id='tree1']")}}
								<div class="sitemap_tree1_wrapper">
									...
									<ul>
										{{foreach from=$xml->getList("page/sitemap/item/item[@id='tree1']/item") item=loop431}}
											<li>
												<a href="{{$loop431->getItemEscape("@link")}}" title="{{$loop431->getItemEscape("@name")}}">{{$loop431->getItemEscape("@name")}}</a>
												{{if  $loop431->getItem("item")}}
													<ul>
														{{foreach from=$loop431->getList("item") item=loop437}}
															<li>
																<a href="{{$loop437->getItemEscape("@link")}}" title="{{$loop437->getItemEscape("@name")}}">{{$loop437->getItemEscape("@name")}}</a>
																{{if  $loop437->getItem("item")}}
																	<ul>
																		{{foreach from=$loop437->getList("item") item=loop443}}
																			<li>
																				<a href="{{$loop443->getItemEscape("@link")}}" title="{{$loop443->getItemEscape("@name")}}">{{$loop443->getItemEscape("@name")}}</a>
																				{{if  $loop443->getItem("item")}}
																					<ul>
																						{{foreach from=$loop443->getList("item") item=loop449}}
																							<li>
																								<a href="{{$loop449->getItemEscape("@link")}}" title="{{$loop449->getItemEscape("@name")}}">{{$loop449->getItemEscape("@name")}}</a>
																							</li>
																						{{/foreach}}
																					</ul>
																				{{/if}}
							...
							{{if  $xml->getItem("page/sitemap/item/item[@id='tree2']")}}
								<div class="sitemap_tree1_wrapper">
									...
									<ul>
										{{foreach from=$xml->getList("page/sitemap/item/item[@id='tree2']/item") item=loop480}}
											<li>
												<a href="{{$loop480->getItemEscape("@link")}}" title="{{$loop480->getItemEscape("@name")}}">{{$loop480->getItemEscape("@name")}}</a>
												{{if  $loop480->getItem("item")}}
													<ul>
														{{foreach from=$loop480->getList("item") item=loop486}}
															<li>
																<a href="{{$loop486->getItemEscape("@link")}}" title="{{$loop486->getItemEscape("@name")}}">{{$loop486->getItemEscape("@name")}}</a>
																{{if  $loop486->getItem("item")}}
																	<ul>
																		{{foreach from=$loop486->getList("item") item=loop492}}
																			<li>
																				<a href="{{$loop492->getItemEscape("@link")}}" title="{{$loop492->getItemEscape("@name")}}">{{$loop492->getItemEscape("@name")}}</a>
																				{{if  $loop492->getItem("item")}}
																					<ul>
																						{{foreach from=$loop492->getList("item") item=loop498}}
																							<li>
																								<a href="{{$loop498->getItemEscape("@link")}}" title="{{$loop498->getItemEscape("@name")}}">{{$loop498->getItemEscape("@name")}}</a>
																							</li>
																						{{/foreach}}
																					</ul>
																				{{/if}}
