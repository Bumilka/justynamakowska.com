						...
						<div id="client-files" class="n68137">
							{{if  $xml->getItem("page/client_files/@mode")  == (string) 'to_download'}}
								<div class="n68137_downloadable">
									<div class="big_label n68137_label">
										<span class="n68137_label">{{$txt['107772::68137_label']}}</span>
									</div>
											...
											<div class="n68137_file">
												...
												<div class="n68137_files_description">
													...
													<div class="n68137_files_description_order">
														<span class="n68137_desc_label">{{$txt['107772::68137_order']}}</span>
													...
													<div class="n68137_files_description_availability">
														<span class="n68137_desc_label">{{$txt['107772::68137_availability']}}</span>
														<span class="n68137_desc_value">
															{{if  $loop272->getItem("@available")  == (string) 'y'}}
																{{$txt['107772::68137_file_available']}}
															{{else}}
																{{$txt['107772::68137_file_unavailable_pay']}}
															{{/if}}
															</span>
													</div>
												</div>
												{{if  $loop272->getItem("@available")  == (string) 'y'}}
													<button class="btn-small n68137_files_download">{{$txt['107772::68137_download']}}
													</button>
												{{else}}
													<a class="n68137_files_pay btn-small" href="{{$loop272->getItemEscape("@order_link")}}">{{$txt['107772::68137_pay']}}</a>
												{{/if}}
												<div class="clearDiv">
												</div>
												<div class="n68137_popup">
													...
													<div class="big_label n68137_label_popup">{{$txt['107772::68137_label_popup']}}
													</div>
													<div class="n68137_description_popup">{{$txt['107772::68137_cms_popup']}}
														<div class="n68137_description_cms_popup">{{$loop272->getItemEscape("/shop/page/cms_txt/cms[@type='toplayer']")}}
														</div>
													</div>
													{{foreach from=$loop272->getList("product_attachments/attachment") item=loop324}}
														<div class="n68137_files_description_popup">
															<div class="n68137_desc_popup">
																<div class="n68137_desc_name_popup">
																	{{if  $loop324->getItem("@name") and  $loop324->getItem("@name")  neq (string) ''}}
																		<span class="n68137_desc_name_popup">{{$loop324->getItemEscape("@name")}}</span>
																	{{/if}}
																	{{if  $loop324->getItem("@size_mb") and  $loop324->getItem("@size_mb")  neq (string) '' and  $loop324->getItem("@size_mb")  gt (string) 0}}
																		<span class="n68137_desc_size_popup">{{$txt['107772::68137_desc_size_popup_bracket_open']}}{{$loop324->getItemEscape("@size_mb")}}{{$txt['107772::68137_desc_size_popup_bracket_close']}}</span>
																	{{/if}}
																</div>
																<div class="n68137_desc_availability_popup">{{$txt['107772::68137_available']}}
																	{{if  $loop324->getItem("@unlimited") == (string)'true'}}
																		{{$txt['107772::68137_unlimited']}}
																		{{if  $loop324->getItem("@expiration_date")}}
																			{{$txt['107772::68137_until']}}{{$loop324->getItemEscape("@expiration_date")}}
																		{{/if}}
																	{{elseif ( $loop324->getItem("@unlimited") == (string)'false') and !( $loop324->getItem("@downloads_limit"))}}
																		{{$txt['107772::68137_unlimited']}}
																		{{if  $loop324->getItem("@expiration_date")}}
																			{{$txt['107772::68137_until']}}{{$loop324->getItemEscape("@expiration_date")}}
																		{{/if}}
																	{{else}}
																		{{$loop324->getItemEscape("@downloads_left")}}
																		{{if  $loop324->getItem("@downloads_left")  == (string) 1}}
																			{{$txt['107772::68137_time']}}
																		{{else}}
																			{{$txt['107772::68137_times']}}
																		{{/if}}
																		{{if  $loop324->getItem("@expiration_date")}}
																			{{$txt['107772::68137_until']}}{{$loop324->getItemEscape("@expiration_date")}}
																		{{/if}}
															...
															<a class="btn-small n68137_files_download_popup" href="{{$loop324->getItemEscape("@download_link")}}">
																<span class="n68137_button_label_popup">{{$txt['107772::68137_button_label_popup']}}
																	<span class="n68137_button_value_popup">{{$loop324->getItemEscape("@file_extension")}}</span></span></a>
															<div class="clearDiv">
															</div>
														</div>
													{{/foreach}}
							...
							{{elseif  $xml->getItem("page/client_files/@mode")  == (string) 'not_available'}}
								<div class="n68137_notdownloadable">
									<div class="big_label n68137_label">
										<span class="n68137_label">{{$txt['107772::68137_label_not_available']}}</span>
									</div>
									<div class="n68137_description">
									...
									<div class="n68137_files">
										{{foreach from=$xml->getList("/shop/page/client_files/product_data") item=loop394}}
											<div class="n68137_file">
												<div class="n68137_files_image">
													<img class="n68137_img" src="{{$loop394->getItemEscape("@icon")}}" alt="{{$loop394->getItemEscape("product_attachments/attachment/@file_name")}}"></img>
												</div>
												<div class="n68137_files_description">
													<a class="n68137_desc_name" href="{{$loop394->getItemEscape("@link")}}">{{$loop394->getItemEscape("@name")}}</a>
													<div class="n68137_files_description_order">
														<span class="n68137_desc_label">{{$txt['107772::68137_order']}}</span>
														<a class="n68137_desc_value" href="{{$loop394->getItemEscape("@order_link")}}">{{$loop394->getItemEscape("@order_sn")}}</a>
													</div>
													<div class="n68137_files_description_availability">
														<span class="n68137_desc_label">{{$txt['107772::68137_availability']}}</span>
														<span class="n68137_desc_value">
															{{if  $loop394->getItem("product_attachments/@downloads_left")  == (string) '0'}}
																{{$txt['107772::68137_file_depleted']}}
															{{else}}
																{{$txt['107772::68137_file_unavailable']}}
															{{/if}}
															</span>
													</div>
												</div>
												{{if  $loop394->getItem("@available") == (string)'y'}}
													<a class="btn-small n68137_files_buy" href="{{$loop394->getItemEscape("@link")}}">{{$txt['107772::68137_buy']}}</a>
												{{/if}}
												<div class="clearDiv">
												</div>
											</div>
