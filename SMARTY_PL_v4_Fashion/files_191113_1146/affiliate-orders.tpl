				...
				{{if !( $xml->getItem("action/set_render/item")) or ( $xml->getItem("action/set_render/item/@name")  == (string) 'content')}}
					<div {{assign "classAttributeTmp2" ""}}  id="content"{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}span9{{/capture}}{{if $xml->getItem("page/@type") == (string) 'projector'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_projector_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'search'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_search_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'main'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_main_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'navigation'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_navigation_page_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-list'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_list_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-item'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_item_c_class']}}{{/capture}}{{else}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_otherwise_c_class']}}{{/capture}}{{/if}}  class="{{$classAttributeTmp2}}">
						...
						{{if !( $xml->getItem("page/@type") == (string)'affiliate-not-active')}}
								...
								<div class="affiliate_orders">
									<div class="affiliate_orders_sub">
										<table {{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}affiliate_orders{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}ui-responsive table-stroke ui-table ui-table-reflow affiliate_orders{{/capture}}  class="{{$classAttributeTmp4}}">
											...
											<tbody>
												{{foreach from=$xml->getList("page/orders/client") item=loop303}}
													<tr {{assign "classAttributeTmp5" ""}}{{if $loop303@iteration % 2}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}affiliate_orders_row1{{/capture}}{{else}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}affiliate_orders_row2{{/capture}}{{/if}}  class="{{$classAttributeTmp5}}">
														<td class="affiliate_orders_first">
															<span class="affiliate_orders_name">
																<a data-mobile-class="btn-small" href="{{$loop303->getItemEscape("@link")}}">{{$loop303->getItemEscape("@nr")}}</a></span>
														</td>
														<td>{{$loop303->getItemEscape("@setDate")}}
														</td>
														<td>{{$loop303->getItemEscape("@totalCount")}}
														</td>
														<td>
															<span class="affiliate_orders_price">{{$loop303->getItemEscape("@totalSum_formatted")}}</span>
														</td>
														<td>{{$loop303->getItemEscape("@sentCount")}}
														</td>
														<td>
															<span class="affiliate_orders_price">{{$loop303->getItemEscape("@sentSum_formatted")}}</span>
														</td>
													</tr>
												{{/foreach}}
							...
							{{/if}}
						{{/if}}
						<!--Zamówienia klientów - stronicowanie (affiliate_orders_paging, 91961.1)-->
							...
							<ul class="pagination pull-right">
								{{if count( $xml->getList("page/orders/navigation/item"))  gt (string) 1}}
									{{if  $xml->getItem("page/orders/navigation/prev")}}
										<li>
											<a href=" {{$xml->getItem("page/orders/navigation/path/@value")}}{{$xml->getItemEscape("page/orders/navigation/prev/@counter")}}">{{$txt['91961::affiliate_orders_002']}}</a>
										</li>
									{{/if}}
									{{if !( $xml->getItem("page/orders/navigation/prev"))}}
										<li class="disabled">
											<span>{{$txt['91961::affiliate_orders_002a']}}</span>
										</li>
									{{/if}}
									{{foreach from=$xml->getList("page/orders/navigation/item") item=loop379}}
										{{if  $loop379->getItem("@gap")}}
											<li class="disabled">
												<span>{{$txt['91961::affiliate_calculations_007']}}</span>
											</li>
										{{/if}}
									...
									{{if  $xml->getItem("page/orders/navigation/next")}}
										<li>
											<a href="{{$xml->getItem("page/orders/navigation/path/@value")}}{{$xml->getItemEscape("page/orders/navigation/next/@counter")}}">{{$txt['91961::affiliate_orders_003']}}</a>
										</li>
									{{/if}}
									{{if !( $xml->getItem("page/orders/navigation/next"))}}
										<li class="disabled">
											<span>{{$txt['91961::affiliate_orders_003a']}}</span>
										</li>
									{{/if}}
								{{/if}}
							</ul>
