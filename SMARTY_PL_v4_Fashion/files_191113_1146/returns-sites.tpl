								...
								<div {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}progress_bar_wrapper{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}progress_bar_wrapper pb_step4{{/capture}}  class="{{$classAttributeTmp3}}">
									...
									<div class="_step _step1">
										<span>
											1</span>
										<p>
											<i></i>{{$txt['107784::step1_txt']}}
										</p>
									</div>
									<div class="_step _step2">
										<span>2</span>
										<p>
											<i></i>{{$txt['107784::step2_txt']}}
										</p>
									</div>
									<div class="_step _step3">
										<span>3</span>
										<p>
											<i></i>{{$txt['107784::step3_txt']}}
										</p>
									</div>
									<div class="_step _step4">
										<span>4</span>
										<p>
											<i></i>{{$txt['107784::step4_txt']}}
										</p>
									</div>
									<div class="_step _step5">
										<span>5</span>
										<p>
											<i></i>{{$txt['107784::step5_txt']}}
										</p>
									</div>
									<div class="_step _step6">
										<span>6</span>
										<p>
											<i></i>{{$txt['107784::step6_txt']}}
										</p>
									</div>
								</div>
								{{if count( $xml->getList("page/pickups/pickup"))  gt (string) 1}}
									<h1 class="big_label return_label">{{$txt['107784::51900_00634stock']}}
									</h1>
								{{else}}
									<h1 class="big_label return_label">{{$txt['107784::51900_00634stock1']}}
									</h1>
								{{/if}}
								<form class="pickup_return_list_form" method="get">
									{{if count( $xml->getList("page/pickups/pickup"))  gt (string) 1}}
										{{foreach from=$xml->getList("page/pickups/pickup") item=loop355}}
											<div {{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}pickupl_return{{/capture}}{{if $loop355->getItem("@checked") == (string) 'true'}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}pickupl_return active{{/capture}}{{/if}} id="ps_{{$loop355->getItemEscape("@value")}}"  class="{{$classAttributeTmp4}}">
												<input style="display:none;" type="radio" name="stock" {{if $loop355->getItem("@checked") == (string) 'true'}} checked="checked"{{/if}} value="{{$loop355->getItemEscape("id")}}"></input>
												<div class="pickupl_return_sub">
													<div class="pickupl_return_header">
														<span class="pickupl_return_name">{{$loop355->getItemEscape("name")}}</span>
													</div>
													<div class="pickupl_return_address">
														<span class="pickup_return_address" style="display:none">{{$loop355->getItemEscape("address")}}</span>{{$loop355->getItemEscape("address")}}
														<span class="pickupl_return_city">{{$loop355->getItemEscape("zipcode")}}{{literal}}{{/literal}}{{$loop355->getItemEscape("city")}}</span>
													</div>
													<div class="pickupl_return_link">
														<a target="_blank" href="{{$loop355->getItemEscape("link")}}" title="{{$txt['107784::pickupl_return_linktitle']}}">{{$txt['107784::pickupl_return_link']}}</a>
													</div>
													{{if  $loop355->getItem("info")}}
														<div class="pickupl_return_info">{{$loop355->getItemEscape("info")}}
														</div>
													{{/if}}
												</div>
											</div>
										{{/foreach}}
									{{else}}
										{{foreach from=$xml->getList("page/pickups/pickup") item=loop395}}
											<div class="pickupl_return_one" id="ps_{{$loop395->getItemEscape("@value")}}">
												<input style="display:none;" type="radio" name="stock" checked="checked" value="{{$loop395->getItemEscape("id")}}"></input>
												<div class="pickupl_return_one_sub">
													<div class="pickupl_return_one_link">
														<a target="_blank" data-tablet-class="btn-small" data-mobile-class="btn-small" href="{{$loop395->getItemEscape("link")}}" title="{{$txt['107784::pickupl_return_one_linktitle']}}">{{$txt['107784::pickupl_return_one_link']}}</a>
													</div>
													<div class="pickupl_return_one_header">
														<span class="pickupl_return_one_name">{{$loop395->getItemEscape("name")}}</span>
													</div>
													<div class="pickupl_return_one_address">{{$loop395->getItemEscape("address")}}{{literal}}{{/literal}}
														<span class="pickupl_return_one_city">{{$loop395->getItemEscape("zipcode")}}{{literal}}{{/literal}}{{$loop395->getItemEscape("city")}}</span>
													</div>
													{{if  $loop395->getItem("info")}}
														<div class="pickupl_return_one_info">{{$loop395->getItemEscape("info")}}
														</div>
													{{/if}}
												</div>
											</div>
									...
									<div class="returns_summary_buttons table_display">
										<div class="btn_wrapper backlink">
											{{if  $xml->getItem("page/backlink/@url")}}
												<a class="btn" href="{{$xml->getItemEscape("page/backlink/@url")}}">{{$txt['107784::order2_button_back']}}</a>
											{{/if}}
										</div>
										<div class="btn_wrapper">
											<button id="return_go_next" type="submit" class="btn-large btn_arrow">{{$txt['107784::50001_10a']}}
											</button>
										</div>
									</div>
								</form>
