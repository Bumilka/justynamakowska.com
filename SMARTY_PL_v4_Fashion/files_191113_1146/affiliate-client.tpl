					...
					<div {{assign "classAttributeTmp2" ""}}  id="content"{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}span9{{/capture}}{{if $xml->getItem("page/@type") == (string) 'projector'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_projector_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'search'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_search_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'main'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_main_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'navigation'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_navigation_page_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-list'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_list_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-item'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_item_c_class']}}{{/capture}}{{else}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_otherwise_c_class']}}{{/capture}}{{/if}}  class="{{$classAttributeTmp2}}">
						...
						{{if  $xml->getItem("page/client/item")}}
							<div {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}affiliate_client_label{{/capture}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}big_label affiliate_client_label{{/capture}}  class="{{$classAttributeTmp3}}">
								<span class="affiliate_client_label">{{$txt['90392::affiliate_client_label']}}{{$xml->getItemEscape("page/client/@id")}}</span>
							</div>
							<div class="affiliate_client">
								<div class="affiliate_client_sub">
									<table {{assign "classAttributeTmp4" ""}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}affiliate_client{{/capture}}{{capture name="classAttributeTmp4" assign="classAttributeTmp4"}}ui-responsive table-stroke ui-table ui-table-reflow affiliate_client{{/capture}}  class="{{$classAttributeTmp4}}">
										<thead>
											<tr>
												<th id="affiliate_client_th1">{{$txt['90392::affiliate_client_th1']}}
												</th>
												<th id="affiliate_client_th2">{{$txt['90392::affiliate_client_th2']}}
												</th>
												<th id="affiliate_client_th3">{{$txt['90392::affiliate_client_th3']}}
												</th>
												<th id="affiliate_client_th4">{{$txt['90392::affiliate_client_th4']}}
												</th>
												<th id="affiliate_client_th5">{{$txt['90392::affiliate_client_th5']}}
												</th>
												<th id="affiliate_client_th6">{{$txt['90392::affiliate_client_th6']}}
												</th>
												<th id="affiliate_client_th7">{{$txt['90392::affiliate_client_th7']}}
												</th>
												<th id="affiliate_client_th8">{{$txt['90392::affiliate_client_th8']}}
												</th>
												<th id="affiliate_client_th9">{{$txt['90392::affiliate_client_th9']}}
												</th>
											</tr>
										</thead>
										<tbody>
											{{foreach from=$xml->getList("page/client/item") item=loop304}}
												<tr {{assign "classAttributeTmp5" ""}}{{if $loop304@iteration % 2}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}affiliate_client_row1{{/capture}}{{else}}{{capture name="classAttributeTmp5" assign="classAttributeTmp5"}}affiliate_client_row2{{/capture}}{{/if}}  class="{{$classAttributeTmp5}}">
													<td class="affiliate_client_first">
														{{if  $loop304->getItem("@orderid")}}
															{{$loop304->getItemEscape("@orderid")}}
														{{else}}
															{{$loop304->getItemEscape("@id")}}
														{{/if}}
													</td>
													<td>{{$loop304->getItemEscape("@date")}}
													</td>
													<td>{{$loop304->getItemEscape("@dispatch")}}
													</td>
													<td>
														<span class="affiliate_client_price">{{$loop304->getItemEscape("@cost_formatted")}}</span>
													</td>
													<td>{{$loop304->getItemEscape("@numprod")}}
													</td>
													<td>
														<span class="affiliate_client_price">{{$loop304->getItemEscape("@prepaid_formatted")}}</span>
													</td>
													<td>{{$loop304->getItemEscape("@status")}}
													</td>
													<td>{{$loop304->getItemEscape("@affiliate")}}
													</td>
													<td>{{$loop304->getItemEscape("@commission_formatted")}}
													</td>
												</tr>
											{{/foreach}}
										</tbody>
						...
						{{/if}}
						<!--Zamówienia klienta - stronicowanie (affiliate_client_paging, 91985.1)-->
							...
							<ul class="pagination pull-right">
								{{if count( $xml->getList("page/client/navigation/item"))  gt (string) 1}}
									{{if  $xml->getItem("page/client/navigation/prev")}}
										<li>
											<a href=" {{$xml->getItem("page/client/navigation/path/@value")}}{{$xml->getItemEscape("page/client/navigation/prev/@counter")}}">{{$txt['91985::affiliate_client_002']}}</a>
										</li>
									{{/if}}
									{{if !( $xml->getItem("page/client/navigation/prev"))}}
										<li class="disabled">
											<span>{{$txt['91985::affiliate_client_002a']}}</span>
										</li>
									{{/if}}
									{{foreach from=$xml->getList("page/client/navigation/item") item=loop379}}
										{{if  $loop379->getItem("@gap")}}
											<li class="disabled">
												<span>{{$txt['91985::affiliate_client_007']}}</span>
											</li>
										{{/if}}
										{{if !( $loop379->getItem("@type")  == (string) 'current') and !( $loop379->getItem("@gap"))}}
											<li>
												<a href="{{$loop379->getItem("../path/@value")}}{{$loop379->getItemEscape("@counter") - 1}}">{{$loop379->getItemEscape("@counter")}}</a>
											</li>
										{{/if}}
										{{if  $loop379->getItem("@type")  == (string) 'current'}}
											<li class="active">
												<span>{{$loop379->getItemEscape("@counter")}}</span>
											</li>
										{{/if}}
									{{/foreach}}
									{{if  $xml->getItem("page/client/navigation/next")}}
										<li>
											<a href="{{$xml->getItem("page/client/navigation/path/@value")}}{{$xml->getItemEscape("page/client/navigation/next/@counter")}}">{{$txt['91985::affiliate_client_003']}}</a>
										</li>
									{{/if}}
									{{if !( $xml->getItem("page/client/navigation/next"))}}
										<li class="disabled">
											<span>{{$txt['91985::affiliate_client_003a']}}</span>
										</li>
									{{/if}}
								{{/if}}
							</ul>
