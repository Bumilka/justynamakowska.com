				...
				{{if !( $xml->getItem("action/set_render/item")) or ( $xml->getItem("action/set_render/item/@name")  == (string) 'content')}}
					<div {{assign "classAttributeTmp2" ""}}  id="content"{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}span9{{/capture}}{{if $xml->getItem("page/@type") == (string) 'projector'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_projector_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'search'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_search_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'main'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_main_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'navigation'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_navigation_page_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-list'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_list_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-item'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_item_c_class']}}{{/capture}}{{else}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_otherwise_c_class']}}{{/capture}}{{/if}}  class="{{$classAttributeTmp2}}">
						...
						<div class="blog_categoriesdescription mb-4">
							<h1 class="big_label">
								{{if  $xml->getItem("page/blogitems/additional/category_list/name") and ( $xml->getItem("page/blogitems/additional/category_list/name")  neq (string) '')}}
									{{$xml->getItem("page/blogitems/additional/category_list/name")}}
								{{else}}
									{{$txt['107750::30022_016wy']}}
								{{/if}}
							</h1>
							{{if !( $xml->getItem("page/blogitems/additional/category_list/description")  == (string) '') and ( $xml->getItem("page/blogitems/additional/category_list/description")) }}
								{{if (( $xml->getItem("/shop/page/blogitems/navigation/item[1]/@counter")  == (string)1) and ( $xml->getItem("/shop/page/blogitems/navigation/item[1]/@type")  == (string)'current'))}}
									<div class="blog_categoriesdescription_sub clearfix">{{$xml->getItem("page/blogitems/additional/category_list/description")}}
									</div>
								{{/if}}
							{{/if}}
						</div>
						<!--Blog - List (blog_list_display, 107751.1)-->
						{{capture name="main_blog_list_see_more" assign="main_blog_list_see_more"}}
							{{$txt['107751::53698_xx']}}
						{{/capture}}
						{{if count( $xml->getList("/shop/page/blogitems/blogitem"))  gt (string) 0}}
							<div id="blog_list" class="mb-4 article_wrapper">
								<div class="row article_sub_wrapper">
									{{foreach from=$xml->getList("/shop/page/blogitems/blogitem") item=loop327}}
										<div class="article_element_wrapper col-md-6 col-12">
											<div class="date datasquare">{{$loop327->getItem("date")}}
											</div>
											{{if  $loop327->getItem("more/@href") and !( $loop327->getItem("more/@href") == (string)'')}}
												<h3 class="article_name_wrapper">
													<a class="article_name" href="{{$loop327->getItemEscape("more/@href")}}" title="{{$loop327->getItem("title")}}">{{$loop327->getItem("title")}}</a>
												</h3>
											{{else}}
												{{if  $loop327->getItem("title")  neq (string) ''}}
													<h3 class="article_name_wrapper">
														<span class="article_name">{{$loop327->getItem("title")}}</span>
													</h3>
												{{/if}}
											{{/if}}
											{{if  $loop327->getItem("image/@src") and  $loop327->getItem("image/@src") neq (string)''}}
												{{if  $loop327->getItem("@link")}}
													{{literal}}
													<a class="article_image_wrapper" href="{{/literal}}{{$loop327->getItemEscape("@link")}}{{literal}}" {{/literal}}{{else}}{{literal}}
<span class="article_image_wrapper" {{/literal}}{{/if}}{{literal}}>{{/literal}}
														<img src="/gfx/__IAI_TEMPLATE_LANG_639_1__/loader.gif?r=1573641886" data-src-small="{{$loop327->getItem("image/@src")}}" data-src="{{$loop327->getItem("image/@src")}}" class="article_image b-lazy" alt="{{$loop327->getItem("title/text()")}}"></img>
														{{if  $loop327->getItem("@link")}}
															{{literal}}</a>{{/literal}}
													{{else}}
											...
											<div class="article_text_wrapper">
												<div class="description_text">{{$loop327->getItem("description")}}
												</div>
												{{if  $loop327->getItem("more/@href") and !( $loop327->getItem("more/@href") == (string)'') and $main_blog_list_see_more }}
													<div class="see_more_wrapper">
														<a class="see_more_link" href="{{$loop327->getItemEscape("more/@href")}}" title="{{$loop327->getItem("title")}}">{{$txt['107751::53698_xx']}}</a>
													</div>
												{{/if}}
											</div>
										</div>
						...
						{{/if}}
						<!--Blog - Paging (blog_list_paging, 107757.1)-->
						{{if count( $xml->getList("page/blogitems/navigation/item"))}}
							<ul class="pagination pull-right">
								{{if count( $xml->getList("page/blogitems/navigation/item"))  gt (string) 1}}
									{{if  $xml->getItem("page/blogitems/navigation/prev")}}
										<li>
											<a href=" {{$xml->getItem("page/blogitems/navigation/path/@value")}}{{$xml->getItemEscape("page/blogitems/navigation/prev/@counter")}}">{{$txt['107757::affiliate_blogitems_002']}}</a>
										</li>
									{{/if}}
									{{if !( $xml->getItem("page/blogitems/navigation/prev"))}}
										<li class="disabled">
											<span>{{$txt['107757::affiliate_blogitems_002a']}}</span>
										</li>
									{{/if}}
									{{foreach from=$xml->getList("page/blogitems/navigation/item") item=loop417}}
										{{if  $loop417->getItem("@gap")}}
											<li class="disabled">
												<span>{{$txt['107757::affiliate_calculations_007']}}</span>
											</li>
										{{/if}}
										{{if !( $loop417->getItem("@type")  == (string) 'current') and !( $loop417->getItem("@gap"))}}
											<li>
												<a href="{{$loop417->getItem("../path/@value")}}{{$loop417->getItemEscape("@counter") - 1}}">{{$loop417->getItemEscape("@counter")}}</a>
											</li>
										{{/if}}
										{{if  $loop417->getItem("@type")  == (string) 'current'}}
											<li class="active">
												<span>{{$loop417->getItemEscape("@counter")}}</span>
											</li>
										{{/if}}
									{{/foreach}}
									{{if  $xml->getItem("page/blogitems/navigation/next")}}
										<li>
											<a href="{{$xml->getItem("page/blogitems/navigation/path/@value")}}{{$xml->getItemEscape("page/blogitems/navigation/next/@counter")}}">{{$txt['107757::affiliate_blogitems_003']}}</a>
										</li>
									{{/if}}
									{{if !( $xml->getItem("page/blogitems/navigation/next"))}}
										<li class="disabled">
											<span>{{$txt['107757::affiliate_blogitems_003a']}}</span>
										</li>
									{{/if}}
								{{/if}}
							</ul>
