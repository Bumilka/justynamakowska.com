			...
			<div id="layout" class="{{$txt['107419::70925_layout_class']}}">
				...
				{{if !( $xml->getItem("action/set_render/item")) or ( $xml->getItem("action/set_render/item/@name")  == (string) 'content')}}
					<div {{assign "classAttributeTmp2" ""}}  id="content"{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}span9{{/capture}}{{if $xml->getItem("page/@type") == (string) 'projector'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_projector_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'search'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_search_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'main'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_main_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'navigation'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_navigation_page_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-list'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_list_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-item'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_item_c_class']}}{{/capture}}{{else}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_otherwise_c_class']}}{{/capture}}{{/if}}  class="{{$classAttributeTmp2}}">
						...
						{{capture name="setLink_navigation" assign="setLink_navigation"}}
							{{$txt['108885::69967_setLink']}}
						{{/capture}}
						{{capture name="setNumber_navigation" assign="setNumber_navigation"}}
							{{$txt['108885::69967_setNumber']}}
						{{/capture}}
						<div id="Navigation" class="navigation_main">
							<div class="navigation_description">
								<h1 class="big_label">{{$xml->getItemEscape("page/description/name/text()")}}
								</h1>
								{{if  $xml->getItem("page/description/icon")}}
									<img src="{{$xml->getItemEscape("page/description/icon")}}" alt="{{$xml->getItemEscape("page/description/@name")}}" title="{{$xml->getItemEscape("page/description/@name")}}"></img>
								{{/if}}
								<div class="navigation_desc">{{$xml->getItem("page/description/desc")}}
								</div>
							</div>
							<div class="navigation_items row mx-0">
								{{foreach from=$xml->getList("page/navigation/item") item=loop285}}
									<a class="navigation_item col-6 col-sm-4 col-md-3" href="{{$loop285->getItemEscape("@link")}}">
										{{if  $loop285->getItem("@gfx")}}
											<div class="navigation_img d-flex justify-content-center align-items-center">
												<img src="{{$loop285->getItemEscape("@gfx")}}" alt="{{$loop285->getItemEscape("@name")}}" title="{{$loop285->getItemEscape("@name")}}"></img>
											</div>
										{{/if}}
										<span class="name">{{$loop285->getItem("@name")}}</span>
										{{if $setNumber_navigation}}
											<span class="count">{{$txt['108885::n69967_002']}}
												<span>{{$loop285->getItemEscape("@product_quantity")}}</span>{{$txt['108885::n69967_002a']}}</span>
										{{/if}}
										</a>
								{{/foreach}}
							</div>
							{{if $setLink_navigation and  $xml->getItem("/shop//item[@ni=/shop/navigation/current/@ni]/@display_all_link")}}
								<a class="see_all btn-small" href=" {{$xml->getItemEscape("/shop//item[@ni=/shop/navigation/current/@ni]/@display_all_link")}} ">{{$txt['108885::n69967_001']}}</a>
							{{/if}}
						</div>
					</div>
				{{/if}}
