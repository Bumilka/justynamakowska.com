			...
			<div id="layout" class="{{$txt['107419::70925_layout_class']}}">
				...
				{{if !( $xml->getItem("action/set_render/item")) or ( $xml->getItem("action/set_render/item/@name")  == (string) 'content')}}
					<div {{assign "classAttributeTmp2" ""}}  id="content"{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}span9{{/capture}}{{if $xml->getItem("page/@type") == (string) 'projector'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_projector_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'search'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_search_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'main'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_main_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'navigation'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_navigation_page_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-list'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_list_c_class']}}{{/capture}}{{elseif $xml->getItem("page/@type") == (string) 'blog-item'}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_blog_item_c_class']}}{{/capture}}{{else}}{{capture name="classAttributeTmp2" assign="classAttributeTmp2"}}{{$txt['107419::70925_otherwise_c_class']}}{{/capture}}{{/if}}  class="{{$classAttributeTmp2}}">
						...
						{{if  $xml->getItem("page/@type")  == (string) 'rma_products' and  $xml->getItem("page/@display")  == (string) 'stock'}}
							<div class="step4a">
								<div class="progress_bar_wrapper pb_step4 rma_progress_bar">
									...
									<div class="_step _step1">
										<span>
											1</span>
										<p>
											<i></i>{{$txt['107861::step1_txt']}}
										</p>
									</div>
									<div class="_step _step2">
										<span>2</span>
										<p>
											<i></i>{{$txt['107861::step2_txt']}}
										</p>
									</div>
									<div class="_step _step3">
										<span>3</span>
										<p>
											<i></i>{{$txt['107861::step3_txt']}}
										</p>
									</div>
									<div class="_step _step4">
										<span>4</span>
										<p>
											<i></i>{{$txt['107861::step4_txt']}}
										</p>
									</div>
									<div class="_step _step5">
										<span>5</span>
										<p>
											<i></i>{{$txt['107861::step5_txt']}}
										</p>
									</div>
									<div class="_step _step6">
										<span>6</span>
										<p>
											<i></i>{{$txt['107861::step6_txt']}}
										</p>
									</div>
								</div>
								{{if count( $xml->getList("page/pickups/pickup"))  gt (string) 1}}
									<h1 class="big_label return_label">{{$txt['107861::51900_00634stock']}}
									</h1>
								{{else}}
									<h1 class="big_label return_label">{{$txt['107861::51900_00634stock1']}}
									</h1>
								{{/if}}
								<form class="pickup_return_list_form" method="get">
									{{if count( $xml->getList("page/pickups/pickup"))  gt (string) 1}}
										{{foreach from=$xml->getList("page/pickups/pickup") item=loop362}}
											<div {{assign "classAttributeTmp3" ""}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}pickupl_return{{/capture}}{{if $loop362->getItem("@checked") == (string) 'true'}}{{capture name="classAttributeTmp3" assign="classAttributeTmp3"}}pickupl_return active{{/capture}}{{/if}} id="ps_{{$loop362->getItemEscape("@value")}}"  class="{{$classAttributeTmp3}}">
												...
												<div class="pickupl_return_sub">
													...
													<div class="pickupl_return_link">
														<a target="_blank" href="{{$loop362->getItemEscape("link")}}" title="{{$txt['107861::pickupl_return_linktitle']}}">{{$txt['107861::pickupl_return_link']}}</a>
													</div>
										...
										{{/foreach}}
									{{else}}
										{{foreach from=$xml->getList("page/pickups/pickup") item=loop396}}
											<div class="pickupl_return_one" id="ps_{{$loop396->getItemEscape("@value")}}">
												<input style="display:none;" type="radio" name="stock" checked="checked" value="{{$loop396->getItemEscape("id")}}"></input>
												<div class="pickupl_return_one_sub">
													<div class="pickupl_return_one_link">
														<a target="_blank" data-tablet-class="btn-small" data-mobile-class="btn-small" href="{{$loop396->getItemEscape("link")}}" title="{{$txt['107861::pickupl_return_one_linktitle']}}">{{$txt['107861::pickupl_return_one_link']}}</a>
													</div>
													<div class="pickupl_return_one_header">
														<span class="pickupl_return_one_name">{{$loop396->getItemEscape("name")}}</span>
													</div>
													<div class="pickupl_return_one_address">{{$loop396->getItemEscape("address")}}{{literal}}{{/literal}}
														<span class="pickupl_return_one_city">{{$loop396->getItemEscape("zipcode")}}{{literal}}{{/literal}}{{$loop396->getItemEscape("city")}}</span>
													</div>
													{{if  $loop396->getItem("info")}}
														<div class="pickupl_return_one_info">{{$loop396->getItemEscape("info")}}
														</div>
													{{/if}}
												</div>
											</div>
									...
									<div class="returns_summary_buttons table_display">
										<div class="btn_wrapper backlink">
											{{if  $xml->getItem("page/backlink/@url")}}
												<a class="btn" href="{{$xml->getItemEscape("page/backlink/@url")}}">{{$txt['107861::order2_button_back']}}</a>
											{{/if}}
										</div>
										<div class="btn_wrapper">
											<button id="return_go_next" type="submit" class="btn-large btn_arrow">{{$txt['107861::50001_10a']}}
											</button>
										</div>
									</div>
								</form>
